...
def main(paths):
    ...
    vax["location"] = "Latvia"
    vax.to_csv(paths.tmp_vax_out_man("Latvia"), index=False)
    ...
    # Vaccines
    vaccines_used_date = df.groupby("date").vaccine.apply(lambda x: ", ".join(sorted(x)))
    # Metrics
    ...
    df = df.merge(vaccines_used_date, on="date")
    df.to_csv(paths.tmp_vax_out("Latvia"), index=False)
